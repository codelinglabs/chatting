...
describe("conversation read and write repositories", () => {
  ...
  it("reads conversation records, access checks, tags, and attachments", async () => {
    mocks.query
      ...
      .mockResolvedValueOnce({ rows: [{ user_id: "user_1", site_name: "Marketing site" }] })
    ...
    await expect(findConversationNotificationContextRow("conv_1")).resolves.toEqual({ user_id: "user_1", site_name: "Marketing site" });
    ...
    expect(mocks.query.mock.calls[10]?.[0]).toContain("FROM message_attachments");
  });
